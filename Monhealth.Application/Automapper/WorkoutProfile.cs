    ...
    public class WorkoutProfile : Profile
    {
        public WorkoutProfile()
        {
            ...
            CreateMap<Workout, WorkoutDto>()
                ...
                .ForMember(dest => dest.Duration, opt => opt.MapFrom(src => src.WorkoutExercises.Sum(we => we.Duration + we.Reps * 2)))
                .ForMember(dest => dest.CaloriesBurned, opt => opt.MapFrom(src => src.WorkoutExercises.Sum(we => (we.Exercise.CaloriesPerMinute * (we.Duration / 60)) + we.Exercise.CaloriesPerMinute * (we.Reps * 2 / 60))));
