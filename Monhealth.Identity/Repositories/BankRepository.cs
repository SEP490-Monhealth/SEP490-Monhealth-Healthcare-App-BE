...
namespace Monhealth.Identity.Repositories
{
    public class BankRepository : GenericRepository<Bank, Guid>, IBankRepository
    {
        ...
        {
            return await _context.Banks.FirstOrDefaultAsync(n => n.BankName.ToLower()
            == bankName.ToLower().Trim());
        }

        public async Task<Bank> GetBankByBankCode(string bankCode)
        {
            return await _context.Banks.FirstOrDefaultAsync(b => b.BankCode.ToLower()
            == bankCode.ToLower().Trim());
        }
    }
}
